...
if (Input::exists()) {
    if (Input::get('report') !== null) {
        ...
        $result = DB::getInstance()->query("SELECT * FROM users WHERE joined BETWEEN '$from' AND '$to' AND $report IS NOT NULL");
    }
    /*else{
       $result = DB::getInstance()->query("SELECT * FROM users WHERE university_id = '$university'"); 
    }*/
...
//start while loop to get data
    foreach ($result->results() as $result) {
        ...
        if (($result->university_id == $university) && ($result->group == 2)) {
